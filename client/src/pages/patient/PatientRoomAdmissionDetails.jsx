...
export default function PatientRoomAdmissionDetails() {
  ...
  useEffect(() => {
    ...
    const loadDetails = async () => {
      ...
      try {
        const data = await patientAppointmentService.getRoomAdmissionDetails(parsedId);
            ...
            >
              ...
              <div className="rounded-2xl bg-white border border-slate-100 shadow-sm p-5">
                ...
                <p className="text-sm text-slate-700 font-medium">
                  {withFallback(admission.patientName || currentUser?.name, "Patient")}
                </p>
                <p className="text-xs text-slate-500 mt-1">
                  {withFallback(admission.patientEmail || currentUser?.email, "No email available")}
                </p>
              </div>
              ...
              <div className="rounded-2xl bg-white border border-slate-100 shadow-sm p-5">
                ...
                <p className="text-sm text-slate-700 font-medium mt-2">
                  {withFallback(admission.doctorName, "Not assigned")}
                </p>
                <p className="text-xs text-slate-500 mt-1">
                  {withFallback(admission.doctorDepartment, "Department not specified")}
                 </p>
                <p className="text-xs text-slate-500 mt-1">
                  {withFallback(admission.doctorSpecialization, "Specialization not specified")}
                </p>
               </div>
             </motion.div>
           </div>
